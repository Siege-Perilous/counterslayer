<script lang="ts">
  ...
  let { boxWallThickness: _boxWallThickness = 3, boxTolerance: _boxTolerance = 0.5 }: Props = $props();
...
{#each snapGuides as guide, guideIdx (guideIdx)}
  {@const geometry = new THREE.BufferGeometry().setFromPoints(
    guide.type === 'vertical'
      ? [new THREE.Vector3(guide.position, 0.5, -guide.start), new THREE.Vector3(guide.position, 0.5, -guide.end)]
      : [new THREE.Vector3(guide.start, 0.5, -guide.position), new THREE.Vector3(guide.end, 0.5, -guide.position)]
  )}
