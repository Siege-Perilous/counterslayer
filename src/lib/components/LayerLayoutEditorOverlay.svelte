<script lang="ts">
  ...
  interface Props {
    ...
    onResetAuto: () => void;
    onRotate: () => void;
    canEdit?: boolean; // If false, hide the Edit Layout button (e.g., only one item)
  }
  ...
  // Get effective dimensions based on item type
  let effectiveDims = $derived.by(() => {
...
{#if !isEditMode}
  <!-- Enter edit mode button (inline) - only show if more than one item -->
  {#if canEdit}
...
{:else}
  ...
  {#if selectedPlacement && effectiveDims}
    <div class="selectionInfo">
      <span class="selectionName">{selectedPlacement.name}</span>
      <span class="selectionType">{selectedItemType === 'box' ? 'Box' : 'Tray'}</span>
      <span class="selectionDims">
        {effectiveDims.width.toFixed(0)} × {effectiveDims.depth.toFixed(0)}mm
      </span>
