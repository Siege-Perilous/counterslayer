 ...
 * Handles grid snapping, face-to-face snapping, and overlap detection for layer items (boxes + loose trays)
 */

import type { SnapGuide } from '$lib/stores/layerLayoutEditor.svelte';
...
export interface LayerItemForSnapping {
  id: string;
  x: number;
  y: number;
...
export function findLayerOverlaps(items: LayerItemForSnapping[]): Array<[string, string]> {
  ...
  for (let i = 0; i < items.length; i++) {
    for (let j = i + 1; j < items.length; j++) {
      if (checkLayerItemOverlap(items[i], items[j])) {
