...
export type { CardSize, CounterBaseShape, CounterShape } from '$lib/types/project';
...
export function getCounterPositions(
  ...
  const getSlotCutoutRadius = (slot: EdgeLoadedSlot): number => Math.min(cutoutMax, slot.slotDepth * cutoutRatio);
...
export function createCounterTray(
  ...
  const createEdgeLoadedPocket = (slot: EdgeLoadedSlot, xPos: number, yPos: number) => {
    ...
    if (isTriangle) {
      ...
      if (slot.orientation === 'crosswise') {
        ...
        const rotated = rotateZ(-Math.PI / 2, rotateX(Math.PI / 2, translate([0, 0, -slot.slotWidth / 2], extruded)));
        return translate([xPos + slot.slotWidth / 2, yPos + slot.slotDepth / 2, triCenterZ], rotated);
      }
    }
    ...
    if (isCircle) {
      ...
      if (slot.orientation === 'crosswise') {
        ...
        const rotated = rotateX(Math.PI / 2, translate([0, 0, -slot.slotDepth / 2], extruded));
        return translate([xPos + slot.slotWidth / 2, yPos + slot.slotDepth / 2, shapeCenterZ], rotated);
      } else {
        // Lengthwise: extrude along slotWidth (X direction), counters stack left-to-right
    ...
    for (let i = 0; i < slots.length; i++) {
      ...
      if (i === 0) {
        ...
        if (isAgainstLeftWall) {
          ...
          fingerCuts.push(
            translate([slotXStart, centeredYStart + slot.slotDepth / 2, 0], createHalfSphereCutout(cutoutRadius))
          );
        }
      }
      ...
      if (i < slots.length - 1) {
        ...
        fingerCuts.push(translate([betweenX, yStart, 0], createHorizontalFingerCutout(cutoutRadius, rowDepth)));
      } else {
  ...
  for (const placement of topLoadedPlacements) {
    ...
    if (counterShape?.baseShape === 'triangle' && !isFrontRow) {
      ...
      pocketShape = translate([pw / 2, pl / 2, 0], rotateZ(Math.PI, translate([-pw / 2, -pl / 2, 0], pocketShape)));
    }
    ...
    pocketCuts.push(translate([placement.xPosition + xOffset, yStart + yOffset, pocketFloorZ], pocketShape));
